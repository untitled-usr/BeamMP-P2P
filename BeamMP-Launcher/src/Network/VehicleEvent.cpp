...
void TCPClientMain(const std::string& IP, int Port) {
    debug("TCPClientMain: connecting to " + IP + ":" + std::to_string(Port));
    LastIP = IP;
    LastPort = Port;
    ...
    if (TCPSock == -1) {
        error("Client: socket failed! Error code: " + std::to_string(WSAGetLastError()));
        WSACleanup();
        return;
    }
    ...
    debug("TCPClientMain: calling connect()...");
    ...
    info("Connected to server at " + IP + ":" + std::to_string(Port));

    char Code = 'C';
    send(TCPSock, &Code, 1, 0);
    debug("TCPClientMain: sent 'C', starting SyncResources");
    SyncResources(TCPSock);
    debug("TCPClientMain: SyncResources done, entering server loop");
    while (!Terminate) {
