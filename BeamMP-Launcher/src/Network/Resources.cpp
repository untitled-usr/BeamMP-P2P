...
std::string Auth(SOCKET Sock) {
    debug("Auth: sending version VC" + GetVer());
    TCPSend("VC" + GetVer(), Sock);

    debug("Auth: waiting for version response...");
    auto Res = TCPRcv(Sock);
    debug("Auth: version response (" + std::to_string(Res.size()) + " bytes): " + Res.substr(0, 40));

    if (Res.empty() || Res[0] == 'E' || Res[0] == 'K') {
        error("Auth: version rejected: " + Res.substr(0, 80));
        Abord();
        CoreSend("L");
        return "";
    }

    debug("Auth: sending PublicKey (" + std::to_string(PublicKey.size()) + " bytes): " + PublicKey);
    ...
    debug("Auth: waiting for auth response...");
    Res = TCPRcv(Sock);
    debug("Auth: auth response (" + std::to_string(Res.size()) + " bytes): " + Res.substr(0, 40));
    if (Res.empty() || Res[0] != 'P') {
        error("Auth: expected P<id>, got: " + Res.substr(0, 80));
        Abord();
        CoreSend("L");
        return "";
    }
    ...
    if (Res.find_first_not_of("0123456789") == std::string::npos) {
        ...
        debug("Auth: got ClientID=" + std::to_string(ClientID));
    } else {
        error("Auth: invalid client id: " + Res);
        Abord();
        CoreSend("L");
        UUl("Authentication failed!");
        return "";
    }
    debug("Auth: sending SR (sync resources)");
    TCPSend("SR", Sock);
    if (Terminate) {
        CoreSend("L");
        return "";
    ...
    debug("Auth: waiting for mod list...");
    Res = TCPRcv(Sock);
    debug("Auth: mod list (" + std::to_string(Res.size()) + " bytes): " + Res.substr(0, 80));

    if (Res[0] == 'E' || Res[0] == 'K') {
        error("Auth: mod list error: " + Res.substr(0, 80));
        Abord();
        CoreSend("L");
        return "";
    }
...
std::vector<char> TCPRcvRaw(SOCKET Sock, uint64_t& GRcv, uint64_t Size) {
    ...
    do {
        ...
        DownloadSpeed = static_cast<float>(megabits_per_s);
...
void NewSyncResources(SOCKET Sock, const std::string& Mods, const std::vector<ModInfo> ModInfos) {
    debug("NewSyncResources: " + std::to_string(ModInfos.size()) + " mods");
    if (ModInfos.empty()) {
        debug("NewSyncResources: no mods, sending L + Done");
        CoreSend("L");
        TCPSend("Done", Sock);
        info("Done!");
        return;
    ...
    int ModNo = 0;
    int TotalMods = static_cast<int>(ModInfos.size());
...
void SyncResources(SOCKET Sock) {
    debug("SyncResources: calling Auth...");
    std::string Ret = Auth(Sock);

    debug("SyncResources: Auth returned (" + std::to_string(Ret.size()) + " bytes): " + Ret.substr(0, 80));
    ...
    if (Ret.starts_with("R")) {
        ...
        debug("SyncResources: ParseModInfos success=" + std::to_string(success) + " count=" + std::to_string(modInfo.size()));
    ...
    if (Ret.empty()) {
        debug("SyncResources: Auth returned empty, aborting");
        return;
    }
