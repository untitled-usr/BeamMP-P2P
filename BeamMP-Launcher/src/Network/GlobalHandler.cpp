...
void GameSend(std::string_view Data) {
    ...
    auto Result = send(CSocket, ToSend.data(), static_cast<int>(ToSend.size()), 0);
    if (Result < 0) {
...
void ServerSend(std::string Data, bool Rel) {
    ...
    if (C == 'N' || C == 'W' || C == 'Y' || C == 'V' || C == 'E' || C == 'C')
        Rel = true;
    if (compressBound(static_cast<uLong>(Data.size())) > 1024)
        Rel = true;
    if (Ack || Rel) {
        if (Ack || DLen > 1000)
            SendLarge(Data);
...
#if defined(_WIN32)
    BOOL reuseAddr = TRUE;
    setsockopt(GSocket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuseAddr), sizeof(reuseAddr));
#elif defined(__linux__) || defined(__APPLE__)
    int reuseAddr = 1;
    setsockopt(GSocket, SOL_SOCKET, SO_REUSEADDR, &reuseAddr, sizeof(reuseAddr));
#endif
...
void ParserAsync(std::string_view Data) {
    ...
    case 'p':
        ...
        return;
    case 'M':
        debug("ParserAsync: received M(map) from server: " + std::string(Data.substr(0, 80)));
        MStatus = Data;
        UlStatus = "Uldone";
        return;
    case 'U':
