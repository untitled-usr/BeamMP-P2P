...
void CheckForUpdates(const std::string& CV) {
    if (options.offline_mode) {
        info("Offline mode: skipping launcher update check.");
        TraceBack++;
        return;
    }
...
fs::path LocalBeamMPSourceDir() {
    // Strict portable mode: only use a BeamMP folder next to the launcher executable.
    const fs::path src = (GetBP() / "BeamMP").lexically_normal();
    std::error_code ec;
    const fs::path normalized = fs::weakly_canonical(src, ec);
    if (!ec && !normalized.empty())
        return normalized;
    return src;
}

bool IsSkippableDevDirName(std::string name) {
    std::transform(name.begin(), name.end(), name.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    return name == ".git" || name == ".github" || name == ".idea";
}

void CopyLocalBeamMPTree(const fs::path& from, const fs::path& to) {
    std::error_code ec;
    if (!fs::exists(from, ec) || !fs::is_directory(from, ec)) {
        const fs::path expected = (GetBP() / "BeamMP").lexically_normal();
        fatal(std::string("Local BeamMP mod folder not found: ") + from.string()
            + " (portable mode expects folder at " + expected.string() + ")");
    }
    if (fs::exists(to))
        fs::remove_all(to, ec);
    fs::create_directories(to, ec);
    if (ec)
        fatal(std::string("Failed to prepare ") + to.string() + ": " + ec.message());

    ec.clear();
    fs::directory_iterator it(from, fs::directory_options::skip_permission_denied, ec);
    if (ec)
        fatal(std::string("Cannot read directory ") + from.string() + ": " + ec.message());
    for (; it != fs::directory_iterator(); it.increment(ec)) {
        if (ec)
            fatal(std::string("Directory iteration failed in ") + from.string() + ": " + ec.message());
        const fs::path& ep = it->path();
        const fs::path rel = ep.filename();
        if (IsSkippableDevDirName(rel.string()))
            continue;
        const fs::path dest = to / rel;
        if (fs::is_directory(ep)) {
            CopyLocalBeamMPTree(ep, dest);
        } else if (fs::is_regular_file(ep)) {
            std::error_code ec2;
            fs::create_directories(dest.parent_path(), ec2);
            fs::copy_file(ep, dest, fs::copy_options::overwrite_existing, ec2);
            if (ec2)
                fatal(std::string("Failed to copy ") + ep.string() + ": " + ec2.message());
        }
    }
}

void RemoveOfficialBeamMPZips() {
    const fs::path game = GetGamePath();
    const fs::path mp = game / "mods" / "multiplayer";
    std::error_code ec;
    for (const char* name : { "BeamMP.zip", "beammp.zip" }) {
        const fs::path zip = mp / name;
...
void PreGame(const beammp_fs_string& GamePath) {
    ...
    const fs::path src = LocalBeamMPSourceDir();
    const fs::path dst = GetGamePath() / "mods" / "unpacked" / "beammp";

    try {
        if (!fs::exists(GetGamePath() / beammp_wide("mods/multiplayer"))) {
            fs::create_directories(GetGamePath() / beammp_wide("mods/multiplayer"));
        }
        RemoveOfficialBeamMPZips();
        info(std::string("Installing local BeamMP mod from ") + src.string() + " -> " + dst.string());
        CopyLocalBeamMPTree(src, dst);
        EnableMP();
    ...
    if (options.offline_mode)
        info("Offline mode: using local BeamMP tree (no official client download).");
    else
        info("Using local BeamMP tree (official client zip download and hash check disabled).");
}
