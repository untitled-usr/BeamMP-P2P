...
void InitOptions(int argc, const char *argv[], Options &options) {
    ...
    while (i < argc) {
        ...
        if (argument == "-p" || argument == "--port") {
            ...
            options.no_update = true;
            options.no_download = true;
        } else if (argument == "--offline-name") {
            if (i + 1 >= argc) {
                error("You must specify a name after `--offline-name`");
            } else {
                std::string n(argv[i + 1]);
                while (!n.empty() && std::isspace(static_cast<unsigned char>(n.front())))
                    n.erase(n.begin());
                while (!n.empty() && std::isspace(static_cast<unsigned char>(n.back())))
                    n.pop_back();
                if (n.empty()) {
                    fatal("Username cannot be empty after `--offline-name`");
                }
                options.offline_username = n;
                i++;
            }
            ...
            std::cout << "USAGE:\n"
                ...
                "\t--offline            LAN mode: no cloud login/updates; identity offline:<name> (use with server OfflineMode)\n"
                "\t--offline-name <n>   Optional: set offline name in launcher (else enter in game). [A-Za-z0-9_], 1–32 chars\n"
