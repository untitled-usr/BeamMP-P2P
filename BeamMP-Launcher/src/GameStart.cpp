...
void StartGame(std::wstring Dir) {
    ...
    debug("StartGame exit(2) path reached, sleeping 5s then exiting");
...
void StartGame(std::string Dir) {
    ...
    std::string app = Dir + "/BeamNG.drive.app/Contents/MacOS/BeamNG";
    if (!fs::exists(app)) {
        error("Could not find BeamNG.drive.app in game directory. Use --no-launch and start BeamNG manually.");
        std::this_thread::sleep_for(std::chrono::seconds(5));
        exit(2);
    }
    std::vector<const char*> argv;
    argv.push_back(app.c_str());
    for (int i = 0; i < options.game_arguments_length; i++) {
        argv.push_back(options.game_arguments[i]);
    }
    argv.push_back(nullptr);
    pid_t pid;
    posix_spawn_file_actions_t file_actions;
    auto st = posix_spawn_file_actions_init(&file_actions);
    if (st != 0) {
        error(std::string("posix_spawn_file_actions_init failed: ") + strerror(errno));
    }
    posix_spawn_file_actions_addclose(&file_actions, STDOUT_FILENO);
    posix_spawn_file_actions_addclose(&file_actions, STDERR_FILENO);
    int result = posix_spawn(&pid, app.c_str(), &file_actions, nullptr, const_cast<char**>(argv.data()), environ);
    ...
    int status = 0;
    waitpid(pid, &status, 0);
    error("Game closed. Launcher exiting soon.");
    std::this_thread::sleep_for(std::chrono::seconds(5));
    exit(2);
}
#endif
