...
struct Options {
    ...
    /// Skip cloud auth/updates; use identity token offline:<OfflineUsername> for LAN servers (ServerConfig OfflineMode).
    bool offline_mode = true;
    /// Set via Launcher.cfg / --offline-name to skip in-game name entry; empty = user must enter in BeamMP.
